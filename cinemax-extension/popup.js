// popup.js - Cinemax Helper Extension Popup (v2.0)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const adBlockerBadge = document.getElementById('adBlockerBadge');
  const adBlockerSubtitle = document.getElementById('adBlockerSubtitle');

  function updateAdBlockerUI(enabled) {
    adBlockerToggle.checked = enabled;
    if (enabled) {
      adBlockerBadge.textContent = 'BẬT';
      adBlockerBadge.classList.remove('off');
      adBlockerSubtitle.textContent = 'Đang chặn popup & quảng cáo';
    } else {
      adBlockerBadge.textContent = 'TẮT';
      adBlockerBadge.classList.add('off');
      adBlockerSubtitle.textContent = 'Quảng cáo popup không bị chặn';
    }
  }

  // Load state from background service worker
  chrome.runtime.sendMessage({ action: 'getAdBlockerState' }, (response) => {
    if (chrome.runtime.lastError) {
      console.warn('[Popup] Could not get ad blocker state:', chrome.runtime.lastError.message);
      return;
    }
  ...
  adBlockerToggle.addEventListener('change', () => {
    const newState = adBlockerToggle.checked;
    chrome.runtime.sendMessage({ action: 'setAdBlockerState', enabled: newState }, (response) => {
      if (chrome.runtime.lastError) {
        console.warn('[Popup] Failed to set ad blocker state:', chrome.runtime.lastError.message);
        // Revert toggle on failure
        adBlockerToggle.checked = !newState;
        return;
      }
      updateAdBlockerUI(response ? response.enabled : newState);
    });
  });
